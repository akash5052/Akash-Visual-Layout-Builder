  ...
  return (
    <div className="akash-visual-layout-builder-screen akash-visual-layout-builder-screen--list">
      <header className="akash-visual-layout-builder-screen__header">
        <div>
          <h1 className="akash-visual-layout-builder-screen__title">{label}</h1>
          <p className="akash-visual-layout-builder-screen__subtitle">
            Manage your {label.toLowerCase()} built with Akash Visual Layout Builder.
          </p>
        </div>
        <button type="button" className="akash-visual-layout-builder-btn akash-visual-layout-builder-btn--primary" onClick={() => setShowCreateModal(true)}>
          + New {singular}
        </button>
      </header>
      ...
      {loading ? (
        ...
        <div className="akash-visual-layout-builder-screen__empty">
          ...
          {statusFilter !== "trash" && (
            <button type="button" className="akash-visual-layout-builder-btn akash-visual-layout-builder-btn--ghost" onClick={() => setShowCreateModal(true)}>
          ...
          <table className="akash-visual-layout-builder-content-table">
            ...
            <tbody>
              {data.items.map((item) => {
                ...
                return (
                  <tr key={item.id} className={selected.has(item.id) ? "akash-visual-layout-builder-content-table__row--selected" : ""}>
                    ...
                    <td>
                      {isTrashed ? (
                        <span className="akash-visual-layout-builder-content-table__title akash-visual-layout-builder-content-table__title--static">
                          {item.title || "(no title)"}
                        </span>
                      ) : (
                        <button type="button" className="akash-visual-layout-builder-content-table__title" onClick={() => onOpen(item.id)}>
                          {item.title || "(no title)"}
                        </button>
                      )}
                    </td>
                    <td>
                      <span className={`akash-visual-layout-builder-status akash-visual-layout-builder-status--${statusClass(item.status)}`}>
                        {displayStatus(item.status)}
                      </span>
                    </td>
                    <td className="akash-visual-layout-builder-content-table__date">{formatListDate(item.modified)}</td>
