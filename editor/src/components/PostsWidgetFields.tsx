...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <div className="epb-visual__section-title">{children}</div>;
}
...
export function PostsWidgetPreview({ widget, pageId = 0 }: { widget: PostsWidget; pageId?: number }) {
  ...
  useEffect(() => {
    ...
    renderPostsWidget(payload, pageId)
      ...
      .catch(() => {
        if (!cancelled) setHtml('<div class="epb-posts__empty">Unable to load posts preview.</div>');
      })
      .finally(() => {
  ...
  if (loading && !html) {
    return <div className="epb-posts__empty">Loading posts…</div>;
  }
