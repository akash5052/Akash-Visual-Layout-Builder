...
class EPB_Settings {
	...
	public static function defaults() {
		return [
			...
			// Tracking link (actual tracking lives in epb_tracking)
	...
	public static function get_ai_settings() {
		...
		if (defined('EPB_GEMINI_MODEL') && EPB_GEMINI_MODEL) {
			$settings['gemini_model'] = EPB_GEMINI_MODEL;
		}

		if (defined('EPB_CLAUDE_MODEL') && EPB_CLAUDE_MODEL) {
			$settings['claude_model'] = EPB_CLAUDE_MODEL;
		}
	...
	public static function has_cloud_ai() {
		$s = self::get_raw();
		return !empty($s['ai_enabled']) && EPB_AI_Client::is_available();
	}
	...
	public static function public_settings() {
		...
		return [
			...
			'ai_client_available'   => EPB_AI_Client::is_available(),
			'ai_client_core'        => EPB_AI_Client::core_available(),
			'connectors_url'        => EPB_AI_Client::connectors_url(),
	...
	public static function save($input) {
		if (!is_array($input)) {
			return [
				...
				'message'  => __('Invalid settings payload.', 'wpvisualx'),
			];
		}
		...
		return [
			'success'  => true,
			'settings' => self::admin_settings(),
			'message'  => __('Settings saved.', 'wpvisualx'),
		];
	}
