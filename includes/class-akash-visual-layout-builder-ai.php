...
class Akash_Visual_Layout_Builder_AI {
...
{\"action\":\"replace\",\"html\":\"<h1>Title</h1>\\n<section>...</section>\",\"css\":\"...all css...\",\"js\":\"...all js...\",\"explanation\":\"Rebuilt the full landing page with a hero, features, and CTA.\"}";
	...
	public static function generate($prompt, $settings = [], $context = null, $page_title = '') {
		$settings = wp_parse_args($settings, Akash_Visual_Layout_Builder_Settings::defaults());
		$errors   = [];
		$context  = self::normalize_context($context);
		$intent   = Akash_Visual_Layout_Builder_AI_Intent::parse($prompt, $context);
		...
		if ($intent['type'] === 'images' && $context && !empty($raw['images_enabled'])) {
			$claude = Akash_Visual_Layout_Builder_AI_Claude::enhance_page_images($prompt, $context, $page_title, $intent);
			if ($claude !== null) {
				return $claude;
			}
			$local = Akash_Visual_Layout_Builder_AI_Editor::apply($prompt, $context, $intent);
		...
		if ($intent['type'] === 'seo_content' || Akash_Visual_Layout_Builder_AI_Intent::is_whole_page_prompt($prompt, $context)) {
			$topic = $intent['topic'] ?: Akash_Visual_Layout_Builder_AI_Intent::extract_content_topic($prompt) ?: Akash_Visual_Layout_Builder_AI_Intent::extract_topic($prompt) ?: $page_title ?: 'professional services';
			$code  = Akash_Visual_Layout_Builder_AI_Content::generate_full_page($topic, $page_title);
			$code  = Akash_Visual_Layout_Builder_AI_Claude::align_with_suggestions($code, $topic, $page_title, $prompt);
		...
		$code = Akash_Visual_Layout_Builder_AI_Templates::generate($prompt, $context);
	...
	private static function try_cloud_ai($enhanced_prompt, $prompt, $context, $intent, &$errors, $page_title = '') {
		$result = Akash_Visual_Layout_Builder_AI_Client::generate_text(
			$enhanced_prompt,
			self::SYSTEM_PROMPT,
			[
				'temperature'  => 0.35,
				'max_tokens'   => 8192,
				'timeout'      => 120,
				'models'       => Akash_Visual_Layout_Builder_AI_Client::preferred_models('auto'),
				'json_schema'  => Akash_Visual_Layout_Builder_AI_Client::page_response_schema(),
			]
		);
		if (!is_wp_error($result)) {
			return self::wrap_from_text($result, 'ai', __('Generated with the WordPress AI Client.', 'akash-visual-layout-builder'), $prompt, $context, $intent, $page_title);
		}
	...
	private static function normalize_context($context) {
		...
		return Akash_Visual_Layout_Builder_Renderer::normalize_code($context);
	}
	...
	private static function build_context_prompt($prompt, $context, $intent, $page_title = '') {
		$parts = [ Akash_Visual_Layout_Builder_AI_Intent::instruction_for_ai($intent) ];
		...
		$section_html    = null;
		$section_heading = '';
		$section_type    = $intent['section_type'] ?? Akash_Visual_Layout_Builder_AI_Intent::parse_section_type(strtolower($prompt));
		...
		if ($context && !empty($context['html'])) {
			...
			if ($target && $target !== 'all') {
				$section_html = Akash_Visual_Layout_Builder_AI_Editor::get_section($context['html'], $target ?: 'last');
				if ($section_html) {
					$section_heading = Akash_Visual_Layout_Builder_AI_Editor::extract_heading($section_html);
				}
			}
		}

		$topic = $intent['topic']
			?: Akash_Visual_Layout_Builder_AI_Intent::extract_image_topic($prompt)
			?: Akash_Visual_Layout_Builder_AI_Intent::extract_content_topic($prompt)
			?: Akash_Visual_Layout_Builder_AI_Intent::extract_topic($prompt)
			?: $page_title;

		$resolved = Akash_Visual_Layout_Builder_AI_Images::resolve_topic(
			$topic,
			$page_title,
			$section_html,
			$context['html'] ?? ''
		...
		if (in_array($intent['type'], [ 'images', 'seo_content', 'add', 'edit' ], true)) {
			$parts[] = Akash_Visual_Layout_Builder_AI_Images::image_brief_for_ai(
				$resolved,
				$page_title,
				$section_type ?: '',
				$section_heading
		...
		if ($section_html && $section_heading) {
			...
			$body = Akash_Visual_Layout_Builder_AI_Images::extract_section_text($section_html);
			if ($body !== '') {
	...
	private static function prevent_duplicate_sections($prompt, $context, $intent) {
		...
		$type = $intent['section_type'] ?? Akash_Visual_Layout_Builder_AI_Intent::parse_section_type(strtolower($prompt));
		if (!$type || !Akash_Visual_Layout_Builder_AI_Templates::page_has_section_type($context['html'], $type)) {
			return $intent;
		}
	...
	private static function wrap_from_text($text, $source, $message, $prompt, $context = null, $intent = null, $page_title = '') {
		...
		$parsed['code'] = Akash_Visual_Layout_Builder_AI_Images::align_code_images($parsed['code'], $topic, $title);
		if (in_array($intent['type'] ?? '', [ 'images', 'seo_content', 'add' ], true)) {
			$parsed['code'] = Akash_Visual_Layout_Builder_AI_Claude::align_with_suggestions($parsed['code'], $topic, $title, $prompt);
		}
	...
	public static function parse_ai_code($response, $fallback_prompt = '') {
		...
		return [
			'code'   => Akash_Visual_Layout_Builder_AI_Templates::generate($prompt),
			'source' => 'smart',
		];
	}
