 ...
 */
class Akash_Visual_Layout_Builder_AI_Client {
	...
	public static function generate_text($user, $system = '', $args = []) {
		if (!self::core_available()) {
			return new WP_Error(
				'akash_visual_layout_builder_ai_client_missing',
				__('Cloud AI requires WordPress 7.0 or later with an AI provider configured under Settings → Connectors.', 'akash-visual-layout-builder')
			);
		}
	...
	public static function preferred_models($mode = 'auto') {
		$settings = Akash_Visual_Layout_Builder_Settings::get_ai_settings();
		$gemini   = !empty($settings['gemini_model']) ? (string) $settings['gemini_model'] : Akash_Visual_Layout_Builder_Settings::DEFAULT_GEMINI_MODEL;
		$claude   = !empty($settings['claude_model']) ? (string) $settings['claude_model'] : Akash_Visual_Layout_Builder_Settings::DEFAULT_CLAUDE_MODEL;
	...
	private static function request($user, $system, $args, $timeout) {
		...
		if (!is_object($builder) || !method_exists($builder, 'generate_text')) {
			return new WP_Error(
				'akash_visual_layout_builder_ai_client_missing',
				__('The WordPress AI Client is not available.', 'akash-visual-layout-builder')
			);
		}
		...
		if ($text === '') {
			return new WP_Error(
				'akash_visual_layout_builder_ai_client_empty',
				__('The AI provider returned an empty response.', 'akash-visual-layout-builder')
			);
		}
