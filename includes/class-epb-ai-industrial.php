 ...
 *
 * Loaded as methods on EPB_AI_Content via require of this file's functions
 * being merged — actually we keep a dedicated class and call from EPB_AI_Content.
 */
class EPB_AI_Industrial {
	...
	public static function generate($topic, $page_title = '', $variant = 'manufacturing') {
		...
		$hero_img = esc_url(EPB_AI_Images::relevant_url($topic, 1800, 1000, 0, $page_title, 'hero', $p['hero_headline']));
		$plant_img = esc_url(EPB_AI_Images::relevant_url($topic, 1200, 800, 1, $page_title, 'about', $p['about_title']));
		$cta_img  = esc_url(EPB_AI_Images::relevant_url($topic, 1600, 800, 5, $page_title, 'hero', $p['cta_title']));

		$gal = '';
		for ($i = 0; $i < 6; $i++) {
			$src = esc_url(EPB_AI_Images::relevant_url($topic, 800, 600, $i + 2, $page_title, 'gallery', $brand));
			$gal .= '<figure class="' . $slug . '-shot" data-epb-reveal data-epb-delay="' . ( $i * 70 ) . '">'
		...
		foreach ($p['features'] as $f) {
			$features .= '<article class="' . $slug . '-cap" data-epb-reveal data-epb-delay="' . ( $fi * 90 ) . '">'
				. '<span class="' . $slug . '-cap__idx">' . esc_html($f['icon']) . '</span>'
				. '<h3>' . esc_html($f['title']) . '</h3>'
				. '<p>' . esc_html($f['text']) . '</p>'
				. '<div class="' . $slug . '-cap__bar" aria-hidden="true"><span data-epb-bar="' . ( 78 + ( $fi * 5 ) ) . '"></span></div>'
				. '</article>';
			++$fi;
		}
		...
		foreach ($steps as $i => $step) {
			$process .= '<li class="' . $slug . '-step" data-epb-reveal data-epb-delay="' . ( $i * 100 ) . '">'
		...
		foreach ($p['about_bullets'] as $b) {
			$bullets .= '<li data-epb-reveal><span></span>' . esc_html($b) . '</li>';
		}
		...
		foreach ($p['testimonials'] as $t) {
			...
			$testimonials .= '<blockquote class="' . $slug . '-quote" data-epb-reveal data-epb-delay="' . ( $ti * 90 ) . '">'
		...
		foreach ($p['faq'] as $item) {
			$open = $qi === 0 ? ' open' : '';
			$faq .= '<details class="' . $slug . '-faq__item" data-epb-reveal' . $open . '>'
		...
		foreach ($stats as $i => $s) {
			$stats_html .= '<div class="' . $slug . '-stat" data-epb-reveal data-epb-delay="' . ( $i * 80 ) . '">'
...
-->
<main class=\"{$slug}-page {$slug}-page--{$theme}\" data-epb-industrial=\"{$theme}\">
  <section class=\"{$slug}-hero\" aria-labelledby=\"{$slug}-h1\">
    ...
    <div class=\"{$slug}-hero__inner\">
      <span class=\"{$slug}-pill\" data-epb-reveal>{$badge}</span>
      <h1 id=\"{$slug}-h1\" data-epb-reveal data-epb-delay=\"70\">{$headline}</h1>
      <p class=\"{$slug}-hero__lead\" data-epb-reveal data-epb-delay=\"130\">{$sub}</p>
      <div class=\"{$slug}-hero__actions\" data-epb-reveal data-epb-delay=\"190\">
  ...
  <section id=\"capabilities\" class=\"{$slug}-caps\">
    <div class=\"{$slug}-wrap\">
      <header class=\"{$slug}-head\" data-epb-reveal>
  ...
  <section id=\"process\" class=\"{$slug}-process\">
    <div class=\"{$slug}-wrap\">
      <header class=\"{$slug}-head {$slug}-head--light\" data-epb-reveal>
  ...
  <section class=\"{$slug}-about\">
    <div class=\"{$slug}-wrap {$slug}-about__grid\">
      <div class=\"{$slug}-about__media\" data-epb-reveal=\"left\">
      ...
      <div class=\"{$slug}-about__copy\" data-epb-reveal=\"right\">
  ...
  <section id=\"contact\" class=\"{$slug}-cta\">
	...
	private static function styles($slug, $a, $a2, $theme) {
		...
		return ".{$slug}-page { --epb-a:{$a}; --epb-a2:{$a2}; --epb-ink:{$ink}; --epb-panel:{$panel}; font-family:'IBM Plex Sans',system-ui,sans-serif; color:#e5e7eb; background:var(--epb-ink); line-height:1.6; width:100%; overflow-x:hidden; }
.{$slug}-wrap { max-width:1180px; margin:0 auto; padding:0 24px; }
.{$slug}-eyebrow { margin:0 0 10px; color:var(--epb-a2); font-family:'IBM Plex Mono',monospace; font-size:.72rem; font-weight:600; letter-spacing:.16em; text-transform:uppercase; }
...
.{$slug}-pill { display:inline-flex; align-items:center; gap:8px; padding:8px 16px; border-radius:4px; border:1px solid color-mix(in srgb, var(--epb-a2) 45%, transparent); background:rgba(255,255,255,.06); backdrop-filter:blur(10px); font-family:'IBM Plex Mono',monospace; font-size:12px; font-weight:600; letter-spacing:.04em; }
...
.{$slug}-btn--primary { background:linear-gradient(135deg,var(--epb-a),var(--epb-a2)); color:#0b1020; box-shadow:0 0 0 1px rgba(255,255,255,.1), 0 12px 40px color-mix(in srgb, var(--epb-a) 35%, transparent); }
.{$slug}-btn--primary:hover { transform:translateY(-2px); box-shadow:0 18px 50px color-mix(in srgb, var(--epb-a) 45%, transparent); }
...
.{$slug}-hero__veil { position:absolute; inset:0; background:
  linear-gradient(120deg, color-mix(in srgb, var(--epb-ink) 92%, transparent) 0%, color-mix(in srgb, var(--epb-ink) 55%, transparent) 48%, color-mix(in srgb, var(--epb-a) 35%, transparent) 100%),
  radial-gradient(circle at 80% 20%, color-mix(in srgb, var(--epb-a2) 40%, transparent), transparent 45%); }
...
.{$slug}-hero__sparks i { position:absolute; width:3px; height:3px; border-radius:50%; background:var(--epb-a2); box-shadow:0 0 12px var(--epb-a2); opacity:0; animation:{$slug}-spark 3.6s linear infinite; }
...
.{$slug}-hero__scan { position:absolute; left:0; right:0; height:2px; background:linear-gradient(90deg, transparent, var(--epb-a2), transparent); top:0; opacity:.55; animation:{$slug}-scan 5.5s ease-in-out infinite; }
...
.{$slug}-hero__meter span::after { content:\"\"; position:absolute; inset:auto 0 0 0; background:linear-gradient(var(--epb-a2), var(--epb-a)); animation:{$slug}-meter 2.4s ease-in-out infinite; }
...
.{$slug}-certs__row span::before { content:\"◆\"; color:var(--epb-a2); margin-right:48px; }
...
.{$slug}-stat strong { display:block; font-size:clamp(1.8rem,3vw,2.5rem); font-weight:700; color:var(--epb-a2); letter-spacing:-.03em; }
...
.{$slug}-cap { position:relative; padding:28px 26px 24px; border:1px solid rgba(148,163,184,.16); border-radius:10px; background:color-mix(in srgb, var(--epb-panel) 88%, transparent); overflow:hidden; transition:transform .35s, border-color .35s, box-shadow .35s; }
.{$slug}-cap:hover { transform:translateY(-6px); border-color:color-mix(in srgb, var(--epb-a) 55%, transparent); box-shadow:0 24px 60px rgba(0,0,0,.35); }
.{$slug}-cap__idx { display:inline-block; margin-bottom:14px; font-family:'IBM Plex Mono',monospace; font-size:12px; font-weight:600; color:var(--epb-a2); letter-spacing:.12em; }
...
.{$slug}-cap__bar span { display:block; height:100%; width:0; background:linear-gradient(90deg, var(--epb-a), var(--epb-a2)); transition:width 1.1s cubic-bezier(.22,1,.36,1); }
.{$slug}-cap.is-visible .{$slug}-cap__bar span, .{$slug}-cap .{$slug}-cap__bar span.is-on { width:var(--epb-bar-w, 85%); }
.{$slug}-process { padding:100px 0; background:linear-gradient(180deg, color-mix(in srgb, var(--epb-a) 12%, var(--epb-ink)), var(--epb-ink)); border-block:1px solid rgba(148,163,184,.12); }
...
.{$slug}-step:not(:last-child)::after { content:\"\"; position:absolute; right:-12px; top:50%; width:12px; height:2px; background:linear-gradient(90deg, var(--epb-a2), transparent); }
.{$slug}-step__num { font-family:'IBM Plex Mono',monospace; color:var(--epb-a2); font-weight:600; font-size:1.1rem; }
...
.{$slug}-about__pulse { position:absolute; inset:auto 18px 18px auto; width:72px; height:72px; border-radius:50%; border:2px solid var(--epb-a2); animation:{$slug}-pulse 2.4s ease-out infinite; }
...
.{$slug}-checks span { width:10px; height:10px; border-radius:2px; background:var(--epb-a2); box-shadow:0 0 12px var(--epb-a2); flex-shrink:0; }
...
.{$slug}-quote footer span { color:var(--epb-a2); font-size:.8rem; }
...
.{$slug}-faq__item[open] { border-color:color-mix(in srgb, var(--epb-a) 55%, transparent); box-shadow:0 12px 36px rgba(0,0,0,.25); }
...
.{$slug}-cta__veil { position:absolute; inset:0; background:linear-gradient(135deg, color-mix(in srgb, var(--epb-a) 88%, #000), color-mix(in srgb, var(--epb-ink) 80%, transparent)); }
...
[data-epb-reveal]{opacity:0;transform:translateY(28px);transition:opacity .85s cubic-bezier(.22,1,.36,1),transform .85s cubic-bezier(.22,1,.36,1);}
[data-epb-reveal=\"left\"]{transform:translateX(-40px);}
[data-epb-reveal=\"right\"]{transform:translateX(40px);}
...
@media(prefers-reduced-motion:reduce){
  ...
  [data-epb-reveal]{opacity:1;transform:none;transition:none;}
	...
	private static function scripts($brand, $desc, $slug) {
		...
		$js  = EPB_AI_Content::motion_js();
		$js .= "(function(){
  var root=document.querySelector('.{$slug}-page');
  if(!root) return;
  root.querySelectorAll('[data-epb-bar]').forEach(function(el){
    var w=parseInt(el.getAttribute('data-epb-bar'),10)||80;
    el.style.setProperty('--epb-bar-w', w+'%');
    var card=el.closest('[data-epb-reveal]');
