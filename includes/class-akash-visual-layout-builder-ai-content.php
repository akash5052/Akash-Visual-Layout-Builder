 ...
 */
class Akash_Visual_Layout_Builder_AI_Content {
	...
	public static function starter_catalog() {
		...
		foreach ($items as &$item) {
			try {
				$item['preview'] = Akash_Visual_Layout_Builder_AI_Images::relevant_url($item['topic'], 960, 640, 0, $item['brand'], 'hero', $item['title']);
				$profile         = Akash_Visual_Layout_Builder_AI_Profiles::get($item['topic'], $item['brand']);
	...
	public static function generate_industrial_page($topic, $page_title = '', $variant = 'manufacturing') {
		return Akash_Visual_Layout_Builder_AI_Industrial::generate($topic, $page_title, $variant);
	}
	...
	public static function generate_full_page($topic, $page_title = '') {
		...
		$hero_img = esc_url(Akash_Visual_Layout_Builder_AI_Images::relevant_url($topic, 1600, 900, 0, $page_title, 'hero', $p['hero_headline']));
		$feat_img = esc_url(Akash_Visual_Layout_Builder_AI_Images::relevant_url($topic, 1000, 720, 1, $page_title, 'features', $p['features_title']));
		$cta_img  = esc_url(Akash_Visual_Layout_Builder_AI_Images::relevant_url($topic, 1400, 700, 5, $page_title, 'hero', $p['cta_title']));

		$gal_items = '';
		for ($i = 0; $i < 6; $i++) {
			$src = esc_url(Akash_Visual_Layout_Builder_AI_Images::relevant_url($topic, 720, 540, $i + 2, $page_title, 'gallery', $brand));
			$alt = esc_attr($brand . ' gallery ' . ( $i + 1 ));
			$gal_items .= '<figure class="' . $slug . '-gallery__item" data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay="' . ( $i * 60 ) . '">'
		...
		$thumbs = '';
		for ($i = 0; $i < 3; $i++) {
			$src = esc_url(Akash_Visual_Layout_Builder_AI_Images::relevant_url($topic, 320, 240, $i + 8, $page_title, 'gallery', $brand));
		...
		foreach ($p['features'] as $f) {
			$features_html .= '<article class="' . $slug . '-feat-card" data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay="' . ( $fi * 80 ) . '">'
		...
		foreach ($p['about_bullets'] as $b) {
			$bullets .= '<li data-akash-visual-layout-builder-reveal><span aria-hidden="true">✓</span> ' . esc_html($b) . '</li>';
		}
		...
		foreach ($p['testimonials'] as $t) {
			...
			$testimonials_html .= '<blockquote class="' . $slug . '-quote" data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay="' . ( $ti * 90 ) . '">'
		...
		foreach ($p['faq'] as $item) {
			$open     = $faq_i === 0 ? ' open' : '';
			$faq_html .= '<details class="' . $slug . '-faq__item" data-akash-visual-layout-builder-reveal' . $open . '>'
...
<main class=\"{$slug}-page\">
  <section class=\"{$slug}-hero\" aria-labelledby=\"{$slug}-hero-h1\">
    ...
    <div class=\"{$slug}-hero__inner\">
      <span class=\"{$slug}-hero__badge\" data-akash-visual-layout-builder-reveal>{$badge}</span>
      <h1 id=\"{$slug}-hero-h1\" data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay=\"80\">{$headline}</h1>
      <p class=\"{$slug}-hero__lead\" data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay=\"140\">{$hero_sub}</p>
  ...
  <section class=\"{$slug}-stats\" aria-label=\"Highlights\">
    <div class=\"{$slug}-wrap {$slug}-stats__grid\">
      <div class=\"{$slug}-stat\" data-akash-visual-layout-builder-reveal><strong data-count=\"12\">0</strong><span>Years experience</span></div>
      <div class=\"{$slug}-stat\" data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay=\"80\"><strong data-count=\"850\">0</strong><span>Happy clients</span></div>
      <div class=\"{$slug}-stat\" data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay=\"160\"><strong data-count=\"98\">0</strong><span>Satisfaction %</span></div>
      <div class=\"{$slug}-stat\" data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay=\"240\"><strong data-count=\"24\">0</strong><span>Awards won</span></div>
    </div>
  </section>

  <section id=\"features\" class=\"{$slug}-features\">
    <div class=\"{$slug}-wrap\">
      <header class=\"{$slug}-section-head\" data-akash-visual-layout-builder-reveal>
  ...
  <section class=\"{$slug}-about\">
    <div class=\"{$slug}-about__grid {$slug}-wrap\">
      <div class=\"{$slug}-about__visual\" data-akash-visual-layout-builder-reveal=\"left\">
      ...
      <div class=\"{$slug}-about__text\" data-akash-visual-layout-builder-reveal=\"right\">
  ...
  <section id=\"contact\" class=\"{$slug}-cta\">
	...
	public static function hero_section($topic, $page_title, $slug) {
		$p        = Akash_Visual_Layout_Builder_AI_Profiles::get($topic, $page_title);
		$img      = esc_url(Akash_Visual_Layout_Builder_AI_Images::relevant_url($topic, 1600, 900, 0, $page_title, 'hero', $p['hero_headline']));
		$headline = esc_html($p['hero_headline']);
		$badge    = esc_html($p['hero_badge']);
  ...
  <div class=\"{$slug}-hero__inner\">
    <span class=\"{$slug}-hero__badge\" data-akash-visual-layout-builder-reveal>{$badge}</span>
    <h1 data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay=\"80\">{$headline}</h1>
    <p data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay=\"140\">{$sub}</p>
    <div class=\"{$slug}-hero__btns\" data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay=\"200\">
...
[data-akash-visual-layout-builder-reveal]{opacity:0;transform:translateY(28px);transition:opacity .75s cubic-bezier(.22,1,.36,1),transform .75s cubic-bezier(.22,1,.36,1);}
...
@keyframes {$slug}-ken { from { transform:scale(1.08) translateY(0); } to { transform:scale(1) translateY(-1.5%); } }";
	...
	public static function features_section($topic, $page_title, $slug) {
		...
		$i     = 0;
		foreach ($p['features'] as $f) {
			$img = esc_url(Akash_Visual_Layout_Builder_AI_Images::relevant_url($topic, 640, 420, $i, $page_title, 'features', $f['title']));
			$cards .= '<article data-akash-visual-layout-builder-reveal data-akash-visual-layout-builder-delay="' . ( $i * 80 ) . '">'
		...
		$html = "<section class=\"{$slug}-feat\"><div class=\"inner\"><h2 data-akash-visual-layout-builder-reveal>{$title}</h2><p class=\"lead\" data-akash-visual-layout-builder-reveal>{$lead}</p><div class=\"grid\">{$cards}</div></div></section>";
...
[data-akash-visual-layout-builder-reveal].is-visible{opacity:1;transform:none;}";
	...
	public static function motion_js() {
		return "(function(){
  if(window.__akashVisualLayoutBuilderMotionInit) return; window.__akashVisualLayoutBuilderMotionInit=true;
  var reduce=window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  var els=document.querySelectorAll('[data-akash-visual-layout-builder-reveal]');
  function show(el){
    var d=parseInt(el.getAttribute('data-akash-visual-layout-builder-delay')||'0',10);
...
[data-akash-visual-layout-builder-reveal]{opacity:0;transform:translateY(28px);transition:opacity .8s cubic-bezier(.22,1,.36,1),transform .8s cubic-bezier(.22,1,.36,1); will-change:opacity,transform;}
[data-akash-visual-layout-builder-reveal=\"left\"]{transform:translateX(-36px);}
[data-akash-visual-layout-builder-reveal=\"right\"]{transform:translateX(36px);}
[data-akash-visual-layout-builder-reveal].is-visible{opacity:1;transform:none;}
