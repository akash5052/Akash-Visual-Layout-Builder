...
class EPB_AI_Editor {
	...
	public static function apply($prompt, $context, $intent) {
		if (empty($context['html']) && in_array($intent['type'], ['edit', 'animate', 'images'], true)) {
			return [
				'code'    => $context,
				'source'  => 'smart',
				'message' => __('Add a section first, then you can edit it.', 'wpvisualx'),
				'action'  => 'none',
			];
		}
		...
		switch ($intent['type']) {
			case 'animate':
				return self::wrap_result(
					...
					__('Animations added to your page.', 'wpvisualx')
				);

			case 'images':
				$topic = $intent['topic'] ?: EPB_AI_Intent::extract_image_topic($prompt);
				...
				if ($result) {
					return self::wrap_result($result, __('Relevant images added to match your page content.', 'wpvisualx'));
				}
				break;
			...
			case 'edit':
				...
				if ($result) {
					$msg = !empty($intent['is_complaint']) ? __('Section fixed and improved.', 'wpvisualx') : __('Section updated.', 'wpvisualx');
	...
	public static function regenerate_section($context, $intent, $prompt) {
		...
		if (!$title || EPB_AI_Intent::is_bad_title($title)) {
		...
		$new  = EPB_AI_Templates::section_by_type($type, $title, $slug, $prompt);
...
@keyframes epb-scale-in {
  from { opacity: 0; transform: scale(0.94); }
  to { opacity: 1; transform: scale(1); }
}
@keyframes epb-slide-left {
  from { opacity: 0; transform: translateX(-40px); }
  to { opacity: 1; transform: translateX(0); }
}

.epb-anim-enter {
  animation: epb-fade-up 0.75s cubic-bezier(0.22, 1, 0.36, 1) forwards;
  opacity: 0;
}
.epb-anim-delay-1 { animation-delay: 0.12s; }
.epb-anim-delay-2 { animation-delay: 0.24s; }
.epb-anim-delay-3 { animation-delay: 0.36s; }
.epb-anim-delay-4 { animation-delay: 0.48s; }
...
.epb-scroll-reveal.epb-revealed {
  opacity: 1;
...
document.addEventListener('DOMContentLoaded', function () {
  var els = document.querySelectorAll('.epb-scroll-reveal');
  if (!els.length) return;
  if (!('IntersectionObserver' in window)) {
    els.forEach(function (el) { el.classList.add('epb-revealed'); });
    return;
  }
  ...
  els.forEach(function (el) { io.observe(el); });
		...
		if ($target === 'all') {
			$html = preg_replace('/<section\b/i', '<section class="epb-scroll-reveal"', $html);
			$html = preg_replace('/class="epb-scroll-reveal"\s+class="/i', 'class="epb-scroll-reveal ', $html);
		} else {
			$section = self::get_section($html, $target);
			if ($section) {
				$updated = preg_replace('/<section\b/i', '<section class="epb-scroll-reveal epb-anim-enter"', $section, 1);
			...
			function ($m) {
				...
				return preg_replace_callback(
					...
					function ($tag) use (&$i) {
						$delay = min($i, 4);
						$i++;
						return '<' . $tag[1] . ' class="epb-anim-enter epb-anim-delay-' . $delay . '"';
					},
					$block,
					6
				);
	...
	public static function inject_images($context, $topic, $target = 'last', $prompt = '', $page_title = '') {
		...
		$section_type = EPB_AI_Intent::parse_section_type(strtolower($prompt));
		if (is_string($target) && strpos($target, 'type:') === 0) {
		...
		if ($target === 'all') {
			...
			if (empty($sections)) {
				...
				foreach ($sections as $sec) {
					$resolved = EPB_AI_Images::resolve_topic($topic, $page_title, $sec, $html);
					$type     = EPB_AI_Images::detect_section_type_from_html($sec);
					$updated  = EPB_AI_Images::align_section_images($sec, $resolved, $page_title, $type);
					if (stripos($updated, '<img') === false && stripos($updated, 'background-image') === false) {
						$industry = EPB_AI_Images::industry_for_topic($resolved, $page_title);
						$heading  = EPB_AI_Editor::extract_heading($sec);
						$alt      = EPB_AI_Images::descriptive_alt($resolved, $industry, 0, $heading);
						$img      = EPB_AI_Images::img_tag($alt, $resolved, 'epb-ai-injected-img', 1000, 560, 0, $page_title, $type, $heading);
						$updated  = preg_replace(
							'/(<section[^>]*>)(\s*)/i',
							'$1$2<div class="epb-ai-img-wrap">' . $img . '</div>',
							$updated,
							1
						);
					}
		...
		if (!$section) {
			$resolved = EPB_AI_Images::resolve_topic($topic, $page_title, null, $html);
			$industry = EPB_AI_Images::industry_for_topic($resolved, $page_title);
			$alt      = EPB_AI_Images::descriptive_alt($resolved, $industry);
			$heading  = EPB_AI_Images::is_generic_topic($resolved)
				? ucfirst(EPB_AI_Images::industry_label($industry))
				: ucfirst($resolved);

			$new_section = '<section class="epb-image-sec">' . "\n"
				. '  <div class="epb-image-sec__inner">' . "\n"
				. '    ' . EPB_AI_Images::img_tag($alt, $resolved, 'epb-image-sec__hero', 1200, 640, 0, $page_title, 'hero', $heading) . "\n"
				. '    <h2>' . esc_html($heading) . '</h2>' . "\n"
				. '  </div>' . "\n"
				. '</section>';

			$css = $context['css'] . "/* --- AI Image Section --- */
.epb-image-sec { padding: 60px 20px; width: 100%; text-align: center; }
.epb-image-sec__inner { max-width: 1100px; margin: 0 auto; }
.epb-image-sec__hero { width: 100%; max-height: 480px; object-fit: cover; border-radius: 16px; box-shadow: 0 8px 32px rgba(0,0,0,.12); margin-bottom: 24px; }
.epb-image-sec h2 { font-size: 2rem; color: #0f172a; margin: 0; }";
		...
		$resolved = EPB_AI_Images::resolve_topic($topic, $page_title, $section, $html);
		$industry = EPB_AI_Images::industry_for_topic($resolved, $page_title);
		$heading  = self::extract_heading($section);
		$body     = EPB_AI_Images::extract_section_text($section);
		$type     = $section_type ?: EPB_AI_Images::detect_section_type_from_html($section);
		$updated  = $section;
		...
		if (stripos($updated, '<img') === false && stripos($updated, 'background-image') === false) {
			...
			$updated = preg_replace_callback(
				...
				function ($m) use ($resolved, $page_title, $type, $heading, $body, &$count) {
					...
					$w   = 800;
					$h   = 500;
					$alt = EPB_AI_Images::descriptive_alt($resolved, '', $count - 1, $heading);
					$src = esc_url(EPB_AI_Images::relevant_url($resolved, $w, $h, $count - 1, $page_title, $type, $heading, $body));
					return '<img class="epb-ai-injected-img" src="' . $src . '" alt="' . esc_attr($alt) . '" width="' . $w . '" height="' . $h . '" loading="lazy" decoding="async" />';
				},
				$updated
			);
			$updated = EPB_AI_Images::align_images_in_markup($updated, $resolved, $page_title, $type, $heading);
		}
	...
	private static function injected_image_css($existing) {
		if (strpos($existing, 'epb-ai-injected-img') !== false) {
  ...
  box-shadow: 0 8px 28px rgba(0,0,0,.15);
	...
	public static function edit_page($context, $prompt, $intent) {
		...
		if (!empty($intent['replace']) || preg_match('/\b(?:replace|rewrite|redesign)\b/i', $p)) {
			...
			$new_section = EPB_AI_Templates::generate($prompt, $context);
